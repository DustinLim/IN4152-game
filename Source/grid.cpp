...
Mesh Grid::getReduxMesh(Mesh mesh, int resolution) {
	//Create a grid that covers the bounding box of the mesh
	double offset = 0.01;
	Vec3Df vecOffset = Vec3Df(offset, offset, offset);
	Grid grid = Grid(mesh.bbOrigin - vecOffset, mesh.bbEdgeSize + 2 * offset, resolution);
	...
	//The map contains a map from cell index to vertex index in simplifiedVertices
